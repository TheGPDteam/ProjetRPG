...
Tableau::Tableau(SDL_Rect rect, Controleur *controleur, bool avec_image, float hauteur_ligne)
    :Affichable{rect}, m_hauteurLigne {hauteur_ligne}, m_avecImage(avec_image), m_controleur{controleur}
{
    m_rectangle.h=0;
}
...
//! \date 22/11/18
//!
Ligne * Tableau::ligneSurvole(std::pair<int, int> coord_souris){
    for (Ligne *l : m_lignes){
...
//! \date 16/12/18
//!
bool Tableau::testAffichageLigneSurvole(std::pair<int, int> coord_souris){
    ...
    if (m_lignePrecedemmentSurvoler != nullptr)   m_lignePrecedemmentSurvoler->definirNumCouleur((positionLigne(m_lignePrecedemmentSurvoler) + 1) % 2 + 1);
...
//!
int Tableau::positionLigne(Ligne * ligne){
    int i = 0;
    for (auto l : m_lignes){
        i++;
        if (l == ligne) break;
...
//! \date 16/12/18
//!
Ligne * Tableau::obtenirLigne(int numero_ligne){
    return m_lignes.at(numero_ligne);
}
...
//! \date 12/12/18
//!
void Tableau::ajouterLigne(std::vector<Affichable*> affichables){
...
//!
//!
void Tableau::ajouterLigne(Humain* perso){
...
void Tableau::ajouterLigne(std::vector<std::string> strings){
    std::vector<Affichable*> tmp;
...
//!
ZoneTexte * Tableau::creeZoneTexte(std::string donnee, COMPORTEMENT_TEXTE comportment, ALIGNEMENT_TEXTE alignement){
...
//! \author Lacoste Dorian
//! \date 15/11/2018
void Tableau::ajouterLigne(Objet* obj){
    std::vector<Affichable *> tmp;
...
//! \author Lacoste Dorian
//! \date 15/11/2018
void Tableau::creerLigne(std::vector<Affichable *> donnees_ligne){
    Ligne *l = new Ligne(donnees_ligne, this->m_controleur, creerRectLigne(), (obtenirNbLignes()%2 == 0) ? 1 : 2, obtenirNbLignes() + 1, m_avecImage);
...
//! \date 22/11/18
//!
SDL_Rect Tableau::creerRectLigne(){
    return creerRectLigne(obtenirNbLignes());
}


SDL_Rect Tableau::creerRectLigne(int numero_ligne){
    SDL_Rect rectangleParLigne = m_rectangle;
    rectangleParLigne.y = this->m_rectangle.y + this->m_hauteurLigne * numero_ligne;
...
ZoneDefilable * Tableau::obtenirZoneDefilableTableau(SDL_Rect rectangle_affichable){
    return new ZoneDefilable(this,SDL_Color{100, 100, 100, 255}, this->m_controleur, true, rectangle_affichable);
}
